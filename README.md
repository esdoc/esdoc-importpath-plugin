...
This is a plugin that converts the import path.
ESDoc displays the import path of class/function to the document.
However, because usually ES6 is transpiled to npm publish, different import path.

For example, ``src/MyClass.js`` will be referred to as in the ESDoc document ``import MyClass from 'my-module/src/MyClass.js'``.
However, in practice it is different from the import path when you use because it is transpiled
...
Also, when writing multi rules, it will also be carried out transformation many times.
